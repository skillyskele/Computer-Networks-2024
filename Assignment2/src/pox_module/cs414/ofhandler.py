...
log = core.getLogger()
FLOOD_DELAY = 5
#default location /home/ubuntu/cs414_lab3/IP_CONFIG
...
class OFHandler (EventMixin):
  def __init__ (self, connection, transparent):
    ...
    self.listenTo(core.cs414_srhandler)
    core.cs414_ofhandler.raiseEvent(RouterInfo(self.sw_info, self.rtable))
  ...
  def _handle_PacketIn (self, event):
    ...
    pkt = event.parse()
    raw_packet = pkt.raw
    core.cs414_ofhandler.raiseEvent(SRPacketIn(raw_packet, event.port))
...
class cs414_ofhandler (EventMixin):
  """
...
def launch (transparent=False):
  ...
  """    
  core.registerNew(cs414_ofhandler, str_to_bool(transparent))
