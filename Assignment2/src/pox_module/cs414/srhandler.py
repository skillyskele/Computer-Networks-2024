...
class SRServerListener(EventMixin):
  ...
  def __init__ (self, address=('127.0.0.1', 8888)):
    port = address[1]
    self.listenTo(core.cs414_ofhandler)
    self.srclients = []
  ...
  def _handle_packet_msg(self, conn, vns_msg):
    ...
    core.cs414_srhandler.raiseEvent(SRPacketOut(pkt, out_port))
...
class cs414_srhandler(EventMixin):
...
def launch (transparent=False):
  ...
  """
  core.registerNew(cs414_srhandler)
